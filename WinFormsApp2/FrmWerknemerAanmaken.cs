...
{
    public partial class FrmWerknemerAanmaken : Form
    {
        public Gebruiker ingelogdeGebruikerWerknemerMaken = new Gebruiker();
        public FrmWerknemerAanmaken()
        {
            InitializeComponent();
        }
        List<GebruikersRol> lstGebruikersrollen = new List<GebruikersRol>();
        List<Gebruiker> LijstGebruikers = new List<Gebruiker>();
        List<Afdeling> lijstMeegegevenAfdelingen = new List<Afdeling>();
        List<Gebruiker> werknemerMetZelfdeAfdeling = new List<Gebruiker>();
        private void FrmWerknemerAanmaken_Load(object sender, EventArgs e)
        {
            GebruikersInlezen();
            RollenInlezen();
            foreach (Gebruiker gebruiker in LijstGebruikers)
            {
                ...
                {
                    foreach (Afdeling afdelingIngelogdeGebruiker in ingelogdeGebruikerWerknemerMaken.LijstAfdelingen)
                    {
                        if (afdelingGebruiker.Naam == afdelingIngelogdeGebruiker.Naam && gebruiker.Guid != ingelogdeGebruikerWerknemerMaken.Guid)
                        {
                            werknemerMetZelfdeAfdeling.Add(gebruiker);
                        }
                    }
                }
            }
            cmbWerknemer.DataSource = null;
            cmbWerknemer.DataSource = werknemerMetZelfdeAfdeling;
            cmbAfdelingen.DataSource = null;
            cmbAfdelingen.DataSource = ingelogdeGebruikerWerknemerMaken.LijstAfdelingen;
            cmbGebruikersRol.DataSource = null;
            cmbGebruikersRol.DataSource = lstGebruikersrollen;
        }
        private void btnAanmaken_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                { //nieuwe lijst maken en dan een lijst met 1 enkele input
                    lijstMeegegevenAfdelingen.Add((Afdeling)cmbAfdelingen.SelectedItem);
                    ...
                    if (cmbWerknemer.Enabled == true)
                    {
                        LijstGebruikers.Remove((Gebruiker)cmbWerknemer.SelectedItem);
                    }
                    GebruikersSchrijven();
                    lijstMeegegevenAfdelingen.Clear();
                    if (cmbWerknemer.Enabled == true)
                    {
                        MessageBox.Show("De gebruiker is succesvol aangepast.", "Succes", MessageBoxButtons.OK);
                    }
                    else
                    {
            ...
            if (System.IO.File.Exists("Gebruikersrollen.xml"))
            {
                ...
                foreach (XmlNode nodeRol in nodeGebruikersrollen)
                {
                    ...
                    if (dezeRol.Naam != "admin")
                    {
                        ...
                        lstGebruikersrollen.Add(dezeRol);
                    }
                }
            }
